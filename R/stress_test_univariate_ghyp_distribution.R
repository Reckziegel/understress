#' Stress-Test the Generalized Hyperbolic Distribution
#'
#' This function implements a stress-test in the GHD by stretching one the parameters
...
#' @examples
#' #
stress_test_univariate_ghyp_distribution <- function(.invariant, .what = c("mu", "sigma", "skew"), .simulations = 300, .symmetric = FALSE) {
  UseMethod("stress_test_univariate_ghyp_distribution", .invariant)
}

#' @rdname stress_test_univariate_ghyp_distribution
#' @export
stress_test_univariate_ghyp_distribution.default <- function(.invariant, .what = c("mu", "sigma", "skew"), .simulations = 300, .symmetric = FALSE) {
...
#' @rdname stress_test_univariate_ghyp_distribution
#' @export
stress_test_univariate_ghyp_distribution.xts <- function(.invariant, .what = c("mu", "sigma", "skew"), .simulations = 300, .symmetric = FALSE) {
  stress_test_univariate_ghyp_distribution_(.invariant = as.matrix(.invariant), .what = .what, .simulations = .simulations, .symmetric = .symmetric)
}
...
#' @keywords internal
stress_test_univariate_ghyp_distribution_ <- function(.invariant, .what = c("mu", "sigma", "skew"), .simulations = 300, .symmetric = .symmetric) {
  ...
  ghyp_fit <- ghyp::fit.ghypuv(data = .invariant, symmetric = .symmetric, silent = TRUE)
  params <- ghyp::coefficients(ghyp_fit)
  mu     <- params$mu
  sigma  <- params$sigma
  skew   <- params$gamma
  ...
  for (i in 1:5) {
    ...
    for (j in 1:.simulations) {
      ...
      if (.what == "mu") {
        W <- ghyp::rgig(n = n, lambda = ghyp_fit@lambda, chi = ghyp_fit@chi, psi = ghyp_fit@psi)
        X <- mu_ + W * ghyp_fit@gamma + sqrt(W) * as.vector(ghyp_fit@sigma) * stats::rnorm(n)
        fit_simul <- ghyp::fit.ghypuv(X, silent = TRUE)
      } else if (.what == "sigma") {
        W <- ghyp::rgig(n = n, lambda = ghyp_fit@lambda, chi = ghyp_fit@chi, psi = ghyp_fit@psi)
        X <- ghyp_fit@mu + W * ghyp_fit@gamma + sqrt(W) * as.vector(sigma_) * stats::rnorm(n)
        fit_simul <- ghyp::fit.ghypuv(X, silent = TRUE)
      } else {
        W <- ghyp::rgig(n = n, lambda = ghyp_fit@lambda, chi = ghyp_fit@chi, psi = ghyp_fit@psi)
        X <- ghyp_fit@mu + W * skews_ + sqrt(W) * as.vector(ghyp_fit@sigma) * stats::rnorm(n)
        fit_simul <- ghyp::fit.ghypuv(X, silent = TRUE)
      }
  ...
  for (i in seq_along(plots)) {
    ...
    plots[[i]] <- patchwork::wrap_plots(p1 / p2) +
      patchwork::plot_annotation(
        ...
        subtitle = "Effects on the parameters of the Generalized Hyperbolic Distribution"
      )
    print(plots[[i]])
