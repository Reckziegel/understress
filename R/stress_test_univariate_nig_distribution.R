#' Stress-Test the Normal-Inverse Gaussian Distribution
#'
...
#' the impact that the shaken parameter causes in all the other parameters.
#'
...
#' @examples
#' #
stress_test_univariate_nig_distribution <- function(.invariant, .what = c("mu", "sigma", "alpha", "skew"), .simulations = 300, .symmetric = FALSE) {
  UseMethod("stress_test_univariate_nig_distribution", .invariant)
}

#' @rdname stress_test_univariate_nig_distribution
#' @export
stress_test_univariate_nig_distribution.default <- function(.invariant, .what = c("mu", "sigma", "alpha", "skew"), .simulations = 300, .symmetric = FALSE) {
  stop(".invariant must be a tibble, xts or a matrix.", call. = FALSE)
}

#' @rdname stress_test_univariate_nig_distribution
#' @export
stress_test_univariate_nig_distribution.tbl <- function(.invariant, .what = c("mu", "sigma", "alpha", "skew"), .simulations = 300, .symmetric = FALSE) {
  ...
  stress_test_univariate_nig_distribution_(.invariant = .invariant, .what = .what, .simulations = .simulations, .symmetric = .symmetric)
}

#' @rdname stress_test_univariate_nig_distribution
#' @export
stress_test_univariate_nig_distribution.xts <-  function(.invariant, .what = c("mu", "sigma", "alpha", "skew"), .simulations = 300, .symmetric = FALSE) {
  stress_test_univariate_nig_distribution_(.invariant = as.matrix(.invariant), .what = .what, .simulations = .simulations, .symmetric = .symmetric)
}

#' @rdname stress_test_univariate_nig_distribution
#' @export
stress_test_univariate_nig_distribution.matrix <- function(.invariant, .what = c("mu", "sigma", "alpha", "skew"), .simulations = 300, .symmetric = FALSE) {
  stress_test_univariate_nig_distribution_(.invariant = .invariant, .what = .what, .simulations = .simulations, .symmetric = .symmetric)
}

#' @rdname stress_test_univariate_nig_distribution
#' @export
stress_test_univariate_nig_distribution.numeric <- function(.invariant, .what = c("mu", "sigma", "alpha", "skew"), .simulations = 300, .symmetric = FALSE) {
  stress_test_univariate_nig_distribution_(.invariant = .invariant, .what = .what, .simulations = .simulations, .symmetric = .symmetric)
}

#' @keywords internal
stress_test_univariate_nig_distribution_ <- function(.invariant, .what = c("mu", "sigma", "alpha", "skew"), .simulations = 300, .symmetric = FALSE) {

  .what <- match.arg(.what, choices = c("mu", "sigma", "alpha", "skew"))
  ...
  nig_fit <- ghyp::fit.NIGuv(data = .invariant, symmetric = .symmetric, silent = TRUE)
  params <- ghyp::coefficients(nig_fit)
  mu     <- params$mu
  sigma  <- params$sigma
  alpha  <- params$alpha.bar
  skew   <- params$gamma
  ...
  if (.what == "mu") {
    ...
    Alphas <- seq(alpha * 0.8, alpha * 1.2, length.out = 5)
  } else if (.what == "skew") {
    Skews <- seq(skew * 0.8, skew * 1.2, length.out = 5)
  } else {
    stop("Parameter currently not implemented. Valid params are: 'mu', 'sigma', 'alpha' and 'skew'.", call. = FALSE)
  }

  stress_loss_alpha  <- matrix(0, nrow = .simulations, ncol = 5)
  stress_inef_alpha  <- vector("numeric", 5)
  stress_bias_alpha  <- vector("numeric", 5)
  stress_error_alpha <- vector("numeric", 5)
  ...
  for (i in 1:5) {
    ...
    Alpha_hats <- vector("numeric", length = .simulations)
    ...
    for (j in 1:.simulations) {
      ...
      if (.what == "mu") {
        W <- ghyp::rgig(n = n, lambda = nig_fit@lambda, chi = nig_fit@chi, psi = nig_fit@psi)
        X <- mu_ + W * nig_fit@gamma + sqrt(W) * as.vector(nig_fit@sigma) * stats::rnorm(n)
        fit_simul <- ghyp::fit.NIGuv(X, silent = TRUE)
      } else if (.what == "sigma") {
        W <- ghyp::rgig(n = n, lambda = nig_fit@lambda, chi = nig_fit@chi, psi = nig_fit@psi)
        X <- nig_fit@mu + W * nig_fit@gamma + sqrt(W) * as.vector(sigma_) * stats::rnorm(n)
        fit_simul <- ghyp::fit.NIGuv(X, silent = TRUE)
      } else if (.what == "alpha") {
        W <- ghyp::rgig(n = n, lambda = nig_fit@lambda, chi = alpha_, psi = alpha_)
        X <- nig_fit@mu + W * nig_fit@gamma + sqrt(W) * as.vector(nig_fit@sigma) * stats::rnorm(n)
        fit_simul <- ghyp::fit.NIGuv(X, silent = TRUE)
      } else {
        W <- ghyp::rgig(n = n, lambda = nig_fit@lambda, chi = nig_fit@chi, psi = nig_fit@psi)
        X <- nig_fit@mu + W * skews_ + sqrt(W) * as.vector(nig_fit@sigma) * stats::rnorm(n)
        fit_simul <- ghyp::fit.NIGuv(X, silent = TRUE)
      }
      ...
      Alpha_hats[j] <- coeffs$alpha.bar
      Skew_hats[j]  <- coeffs$gamma

    }

    # alpha Analysis
    stress_loss_alpha[ , i] <- (Alpha_hats - alpha) ^ 2
    stress_inef_alpha[i]   <- (stats::sd(Alpha_hats) * ((.simulations - 1) / .simulations)) ^ 2
    stress_bias_alpha[i]   <- (mean(Alpha_hats) - alpha) ^ 2
    stress_error_alpha[i]  <- mean(stress_loss_alpha[ , i])
  ...
  exclusion <- stress_loss_alpha > stats::quantile(stress_loss_alpha, probs = 0.95)
  stress_loss_alpha <- stress_loss_alpha[apply(exclusion, 1, sum) == 0, ]
  ...
  data_plots <- list(mu    = list(stress_loss_mu, stress_bias_mu, stress_inef_mu),
                     ...
                     alpha = list(stress_loss_alpha, stress_bias_alpha, stress_inef_alpha),
  ...
  plots <- vector("list", 4)
  names <- c("Location", "Dispersion", "Alpha", "Skewness")
  for (i in seq_along(plots)) {
    ...
    plots[[i]] <- patchwork::wrap_plots(p1 / p2) +
      patchwork::plot_annotation(
        ...
        subtitle = "Effects on the parameters of the NIG Distribution"
      )
    print(plots[[i]])
